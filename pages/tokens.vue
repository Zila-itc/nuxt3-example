<template>
  <div class="wrapper is-flex is-justify-content-center has-background-dark">
    ...
    >
      ...
      <div v-if="userData?.tokens">{{ userData.tokens }} Tokens</div>
      ...
      <div
        ...
        v-if="completedWords.length == guesses || !result.includes(false)"
      >
        <a href="/tokens" class="button is-warning is-large my-2">
          <b>PLAY AGAIN</b>
        </a>
...
onMounted(async () => {
  ...
  watchEffect(async () => {
    if (
      ...
      //   console.log(correctLetters, winnings)

      // console.log(winnings, userData.value.tokens);
      userData.value.tokens += winnings;

      if (userData.value.tokens <= 0) {
        userData.value.tokens = 7;
      }
